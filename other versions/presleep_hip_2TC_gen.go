...
type Sim struct {
	...
	PoolVocab    *etable.Table   `view:"no-inline" desc:"pool patterns vocabulary"`
	BlankDT      *etable.Table   `view:"no-inline" desc:"blank table to fill in arg slot for TrainAB, TrainAC and TestLure"`
	TrainClose   *etable.Table   `view:"no-inline" desc:"Close training patterns to use"`
	TrainCloseAB *etable.Table   `view:"no-inline" desc:"Close AB training patterns to combine with TrainCloseAC"`
	TrainCloseAC *etable.Table   `view:"no-inline" desc:"Close AC training patterns to combine with TrainCloseAB"`
...
func (ss *Sim) New() {
	ss.PoolVocab = &etable.Table{}
	ss.TrainCloseAB = &etable.Table{}
	ss.TrainCloseAC = &etable.Table{}
...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	...
	net.ConnectLayers(ecout, out, prjn.NewOneToOne(), emer.Forward)
	net.ConnectLayers(out, ecin, prjn.NewOneToOne(), emer.Forward)
...
// ConfigPats configures patterns for the model to run dynamically
// See important example codes inside
func (ss *Sim) ConfigPats(dt, poolVocab, trainPat *etable.Table, name, desc string, listSize, ySize, xSize, poolY, poolX int, design, shuffleVar []string) {
	dt.SetMetaData("name", name)
	dt.SetMetaData("desc", desc)
	dt.SetFromSchema(etable.Schema{
		{"Name", etensor.STRING, []int{1}, []string{"Name"}},
		{"Input", etensor.FLOAT32, []int{ySize, xSize, poolY, poolX}, []string{"ySize", "xSize", "poolY", "poolX"}},
		{"ECout", etensor.FLOAT32, []int{ySize, xSize, poolY, poolX}, []string{"ySize", "xSize", "poolY", "poolX"}},
	}, listSize)

	// first context slot
	nContext := 0
	for iPool := 0; iPool < ySize*xSize; iPool++ {
		if len(design[iPool]) == 5 && design[iPool][0:4] == "ctxt" {
			nContext++
		}
	}
	iFirstContext := ySize*xSize - nContext

	// shuffle
	tmpPoolVocab := poolVocab.Clone()
	for _, shuffleVarNm := range shuffleVar {
		rowIdx := make([]int, listSize)
		for i := range rowIdx {
			rowIdx[i] = i
		}
		erand.PermuteInts(rowIdx)
		for row := 0; row < listSize; row++ {
			tmpPoolVocab.CellTensor(shuffleVarNm, row).CopyFrom(poolVocab.CellTensor(shuffleVarNm, rowIdx[row]))
		}
	}

	// copy pats into subspace
	for row := 0; row < listSize; row++ {
		dt.CellTensor("Name", row).SetString([]int{0}, fmt.Sprint(name, row))
		npool := 0
		for pooly := 0; pooly < ySize; pooly++ {
			for poolx := 0; poolx < xSize; poolx++ {
				dttmpOut := dt.CellTensor("ECout", row).SubSpace([]int{pooly, poolx})
				dttmpInp := dt.CellTensor("Input", row).SubSpace([]int{pooly, poolx})

				if len(design[npool]) == 5 && design[npool][0:4] == "ctxt" {
					tmpPool := tmpPoolVocab.CellTensor(design[npool], npool-iFirstContext)
					if "Train" == name[:5] || "Lure" == name[len(name)-4:len(name)] {
						dttmpOut.CopyFrom(tmpPool)
						// patgen.FlipBits(dttmpOut, 0, 0, 1, 0)
						dttmpInp.CopyFrom(dttmpOut)
					} else {
						dttmpOut.CopyFrom(trainPat.CellTensor("ECout", row).SubSpace([]int{pooly, poolx}))
						dttmpInp.CopyFrom(dttmpOut)
					}
				} else {
					tmpPool := tmpPoolVocab.CellTensor(design[npool], row)
					dttmpOut.CopyFrom(tmpPool)
					if "Train" == name[:5] || design[npool] == "A" || design[npool] == "lA" || design[npool] == "B'" || design[npool] == "C'" {
						dttmpInp.CopyFrom(dttmpOut)
					} else {
						dttmpInp.SetZeros()
					}
...
func (ss *Sim) ConfigVocab(dt *etable.Table, poolY, poolX int, ecPctAct float32) {
	nOn := int(float32(poolY) * float32(poolX) * ecPctAct)
	dt.SetFromSchema(etable.Schema{
		{"void", etensor.FLOAT32, []int{poolY, poolX}, []string{"Y", "X"}},
		{"A", etensor.FLOAT32, []int{poolY, poolX}, []string{"Y", "X"}},
		{"B", etensor.FLOAT32, []int{poolY, poolX}, []string{"Y", "X"}},
		{"C", etensor.FLOAT32, []int{poolY, poolX}, []string{"Y", "X"}},
		{"B'", etensor.FLOAT32, []int{poolY, poolX}, []string{"Y", "X"}},
		{"C'", etensor.FLOAT32, []int{poolY, poolX}, []string{"Y", "X"}},
		{"lA", etensor.FLOAT32, []int{poolY, poolX}, []string{"Y", "X"}},
		{"lB", etensor.FLOAT32, []int{poolY, poolX}, []string{"Y", "X"}},
		{"ctxt1", etensor.FLOAT32, []int{poolY, poolX}, []string{"Y", "X"}},
		{"ctxt2", etensor.FLOAT32, []int{poolY, poolX}, []string{"Y", "X"}},
		{"ctxt3", etensor.FLOAT32, []int{poolY, poolX}, []string{"Y", "X"}},
	}, 1000)
	for i := range dt.Cols {
		if dt.ColName(i) != "void" {
			patgen.PermutedBinaryRows(dt.Cols[i], nOn, 1, 0)
		}
	}
}

// CombineDT combines two tables
func (ss *Sim) CombineDT(dt1, dt2 *etable.Table) *etable.Table {
	cbt := dt1.Clone()
	cbt.AddRows(dt2.NumRows())
	if dt1.NumCols() == dt2.NumCols() {
		for iCol := range cbt.Cols {
			for iRow := 0; iRow < dt2.NumRows(); iRow++ {
				colNm := cbt.ColName(iCol)
				cbt.CopyCell(colNm, iRow+dt1.NumRows(), dt2, colNm, iRow)
			}
		}
		return cbt
	} else {
...
func (ss *Sim) OpenPats() {
	...
	ss.ConfigVocab(ss.PoolVocab, 7, 7, 0.2) // get all permuted pools into ss.PoolVocab
	ss.ConfigPats(ss.TrainCloseAB, ss.PoolVocab, ss.BlankDT, "Train_AB", "AB Training Patterns", 10, 3, 2, 7, 7, []string{"A", "B'", "B", "void", "ctxt1", "ctxt1"}, []string{})
	ss.ConfigPats(ss.TrainCloseAC, ss.PoolVocab, ss.BlankDT, "Train_AC", "AC Training Patterns", 10, 3, 2, 7, 7, []string{"A", "C'", "void", "C", "ctxt1", "ctxt1"}, []string{})
	ss.TrainClose = ss.CombineDT(ss.TrainCloseAB, ss.TrainCloseAC)
